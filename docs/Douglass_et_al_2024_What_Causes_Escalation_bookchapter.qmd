...
p_Figure1 <- temp %>%
  ...
  theme(text = element_text(family = "Courier")) + # Monospaced font
  labs(y = "Behavior", 
...
ggsave("/media/skynet3/8tb_a/rwd_github_private/ICBe_WhatCausesEscalation/docs/Figure1_Behavior_Locations.png", plot = p_Figure1, width = 8.5, height = 11, bg="white")
...
# Function to assign symbols based on the value of 'shap'
assign_symbol <- function(shap) {
  if (shap > 0) {
    return("◤")
  } else if (shap < 0) {
    return("◢")
...
# Function to assign grayscale based on the value of 'shap'
assign_gray_scale <- function(shap, min_shap, max_shap) {
  # Normalize 'shap' to a scale of 0 to 1
  normalized_shap <- (shap - min_shap) / (max_shap - min_shap)
  # Convert to grayscale
  grayscale_value <- 255 - (normalized_shap * 255)
  return(grayscale_value)
}

# Apply the functions to the dataframe
temp1 <- temp1 %>%
  mutate(symbol = sapply(shap, assign_symbol),
         grayscale = sapply(shap, assign_gray_scale, min(temp1$shap), max(temp1$shap)))

# Plotting (this is a conceptual example and might need adjustments)
p_Figure2_bw <- 
  temp1 %>%
  mutate(category = "Error") %>%
  mutate(category = ifelse(color_category %in% "red","Escalatory/Uncooperative",category)) %>%
  ...
  mutate(category = ifelse(color_category %in% "darkorange","Thought",category)) %>%
  mutate(category = ifelse(color_category %in% "purple","Details",category)) %>%
  ggplot( aes(x = factor(value), y = feature)) +
  geom_text(aes(label = symbol, color = shap), size=3) +
  #scale_color_gradient(low = "white", high = "black") +
  scale_color_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 0, limit = c(min(temp1$shap), max(temp1$shap))) +
  theme_bw() +
  theme(text = element_text(family = "Courier")) + # Monospaced font
  # Additional theming and labelling as per your original plot
    theme(
      #axis.text.y = element_text(colour = temp2$color_category),
...
#  theme(
#  legend.position = c(0.95, 0.5),  # Adjust these values as needed
...
ggsave("/media/skynet3/8tb_a/rwd_github_private/ICBe_WhatCausesEscalation/docs/Figure2_LightGBM_Results_bw.png", plot = p_Figure2_bw, width = 8.5, height = 11, bg="white")
...
temp3 <- temp2 %>% left_join(temp1 %>% dplyr::select(crisno, t, feature_clean, shap)) %>%
         mutate(feature_clean = paste0(feature_clean, " (", ifelse(shap>0,"+",""),shap %>% round(1), ")" ) )
...
p_Figure3 <- plot_grid(p1 + theme(text = element_text(family = "Courier")), 
                       p3 + theme(text = element_text(family = "Courier")),
                       p2 + theme(text = element_text(family = "Courier")),
...
p_Figure4 <- cowplot::ggdraw() +
  cowplot::draw_plot(p4 + theme(text = element_text(family = "Courier")) ) +
...
```{r, eval=T, fig.width=8.5, fig.height=12, output=T}
#| fig-cap: "Figure 1: The average location within each narrative that a behavior/detail tends to appear across the whole corpus."
p_Figure1
```
...
```{r, eval=T, fig.width=8.5, fig.height=12, output=T}
#| fig-cap: "Figure 2: The predicted change in number or remaining events in the crisis as a function of time since each behavior/detail. Number of remaining events estimated with LightGBM, and out of sample expected change estimated as SHAP values."
p_Figure2_bw
```

{{< pagebreak >}}
```{r, eval=T, fig.width=6.5*2.5, fig.height=6.5*2, output=T}
#| fig-cap: "Figure 3: Predicted remaining events in each crisis at each time point. Out of sample predictions from the LightGBM model presented in Figure 2. Behavior/detail with the largest impact on the predicted number of remaining events shown at each step along with SHAP value in parentheses."
print(p_Figure3)
```

{{< pagebreak >}}
```{r, eval=T, fig.width=12, fig.height=8.5, output=T}
#| fig-cap: "Figure 4: Crises paths over time clustered into 6 archetypal shapes. Values are the out of sample predicted remaining number or events at each step within each crisis. Overall shape illustrates the path of the crisis, e.g. immediate steady resolution in cluster 1, resolution and then reversion in cluster 3, lengthy stair-step down toward resolution in cluster 6, etc."
